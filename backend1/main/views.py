...
import json
from backend1.main.admin import DoramaAdmin
from main.models import Dorama, Film, User, Comment
...
@api_view(['GET'])
def dorama_list(request):
    s = Dorama.objects.all()
    serializer = DoramaSerializer(DoramaAdmin, many=True)
    return Response(serializer.data, status=status.HTTP_200_OK)

@api_view(['GET'])
def dorama_info(request, dorama_name):
    dorama = Dorama.objects.all().get(name=dorama_name)
...
class UserAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            data = valid_data['user'].user
            data.liked_dorams.add(Dorama.objects.all().get(pk=json_data['dorama']))
            data.save()
            serializer = UserSerializer(instance=data)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except Exception as e:
    ...
    def delete(self, request):
        try:
            ...
            json_data = json.loads(request.body)
            data = valid_data['user'].user
            data.liked_dorama.remove(Dorama.objects.all().get(pk=json_data['dorama']))
            data.save()
            serializer = UserSerializer(instance=data)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except Exception as e:
    ...
    def post(self, request):
        try:
            ...
            data = {
                ...
                'body': json_data['body'],
                'dorama': Dorama.objects.all().get(pk=json_data['dorama'])
            }
